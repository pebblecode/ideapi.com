      ...
      module InstanceMethods
       ...
       # Returns a set of itself and all of its nested children
        def full_set(options = {})
          options.reverse_merge!(:conditions => ["#{scope_condition} AND (#{left_col_name} BETWEEN ? and ?)", self[left_col_name], self[right_col_name]])
          self.class.base_class.find(:all, options)
        end

        # Returns a set of all of its children and nested children
        def all_children
          options.reverse_merge!(:conditions => ["#{scope_condition} AND (#{left_col_name} > ?) and (#{right_col_name} < ?)", self[left_col_name], self[right_col_name]])
          self.class.base_class.find(:all, options)
        end

        # Returns a set of only this entry's immediate children
        def direct_children(options = {})
          options.reverse_merge!(:conditions => ["#{scope_condition} and #{parent_column} = ?", self.id], :order => left_col_name)
          self.class.base_class.find(:all, options)
        end
