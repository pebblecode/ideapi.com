config = BriefConfig.seed(:title) do |s|
  s.title = "Version 0.1"
end

template = BriefTemplate.seed(:title) do |s|
  s.title = "Default"
  s.brief_config_id = config
end

if template

  %w(What? Why? Who? How? Where?).each do |section_title|
    BriefSection.seed(:title) do |s|  
      s.title = section_title
      s.assign_brief_template = template
    end
  end
