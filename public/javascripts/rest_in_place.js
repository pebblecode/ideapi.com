jQuery.fn.rest_in_place = function(url, objectName, attributeName, extraData) {
  var e = this;
  function clickFunction() {
    ...
    e.find("form").submit(function(){
      ...
      jQuery.ajax({
        "url" : url,
        "type" : "post",
        "data" : "_method=put&"+objectName+'['+attributeName+']='+encodeURIComponent(value)+(window.rails_authenticity_token ? "&authenticity_token="+encodeURIComponent(window.rails_authenticity_token) : '')+extraData,
        "success" : function(){
          jQuery.ajax({
            "url" : url,
...
jQuery(function(){
  jQuery(".rest_in_place").each(function(){
    ...
    extras = e.attr("extras")    || extras;
    
    e.rest_in_place(url, obj, attrib, extras);
  });
});